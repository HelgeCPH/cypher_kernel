...
df = pd.DataFrame()"""
    ...
    def start_cypher_shell(self):
        ...
        cypher_shell_bin = os.path.join(os.path.dirname(os.path.abspath(__file__)), cypher_shell_bin)
        cypher = REPLWrapper(
    ...
    def _parse_config():
        ...
        try:
            ...
            if (
                len([k for k in ["user", "pwd", "host", "connect_result_nodes", "cmd_timeout"] if k in config.keys()])
                == 5
            ):
                return config
    ...
    def _response_to_js_graph(self, nodes, relations, elemen_id, node_types=[]):
...
"""
        ...
        graphJS = template.render(nodes=nodes, rels=relations, elemen_id=elemen_id, node_colors=self.global_node_colors)
    ...
    def _send_query_to_cypher_shell(self, code):
        ...
        res = self.cypher_shell.run_command(code, timeout=self.cmd_timeout).splitlines()
